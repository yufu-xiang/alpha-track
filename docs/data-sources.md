...
- 注意事項:
  ...
  - **對不上分割比的跳空:2026-08-27 找到可靠的判別法。** 先前實測過三種都不夠:
    | 判別法 | 為何不夠 |
    |---|---|
    | 單一幅度門檻 | 00715L(布蘭特原油正2)不受漲跌幅限制,全序列有五天超過 35%,會被誤判 |
    | 成交金額連續性 | 對流動性同時劇變的標的失效(00715L 2025-12-01 量與額同時腰斬) |
    | 價位持續性(**30 日**中位數) | 持續的多頭行情看起來與尺度改變相同(00715L 30 日中位價 24.53 → 51.95) |

    **可行的版本是把窗口縮到 5 個交易日,並且前後都取中位數。** 30 天之所以
    失敗,是因為那段長度足以讓一段真實的多頭行情把價位帶到另一個水位;
    5 天不夠讓行情走那麼遠,卻足以看出「跳完之後有沒有回來」。
    判準是「跳空後的水位距**新**水位比距**舊**水位近 5 倍以上」,實測分離度極大:

    | 代號 | 日期 | 倍率 | 距舊/距新 | 判定 |
    |---|---|---|---|---|
    | 00631L | 2015-01-05 | 21.92 | **455** | 尺度改變 → 截斷 |
    | 00715L | 2026-03-09 | 1.62 | 1.45 | 真實行情 → 保留 |
    | 00633L | 2015-02-06 | 1.44 | 0.87 | 壞跳點 → 保留 |

    **前後都取中位數這一點是實測逼出來的**:00633L 在 2015-02-05 有一個孤立的
    壞點(36.77 → **24.50** → 35.16),只拿跳空前一天當基準的話,
    2015-02-06 從 24.50 跳回 35.16 會被判成「新水位持續」,於是砍掉三個月歷史。
    取中位數之後「跳空前的水位」是 35.33 而非 24.50,結論就反過來了。

    成果:00631L 的 MDD 自 −96.9% 修正為 **−55.1%**(2 倍槓桿在 2022 年空頭的合理值),
    `data_start` 變成 2015-01-05、晚於掛牌日,因此「成立以來」正確地為 null。
    全站重新掃描確認沒有其他標的受影響。
    仍有六檔 MDD < −85%,但它們**沒有斷點** —— 反向與槓桿型在多頭十年裡
    確實會因波動耗損跌掉九成,那是正確資訊而非壞資料。
...
### TWSE 舊站 — 除權除息計算結果表(TWT49U)
- URL: `https://www.twse.com.tw/rwd/zh/exRight/TWT49U?startDate=YYYYMMDD&endDate=YYYYMMDD&response=json`
- 方法: GET,無需金鑰
- 回應結構: `{stat, fields, data}`,`data` 為 `list[list[str]]`。一次請求可取**整段區間的全市場**紀錄(實測 2024 全年 1184 筆,其中 232 筆是 ETF),因此回補 2015 年起的十二年只要十二次請求
- Fixture: `pipeline/tests/fixtures/twse_ex_rights_2024.json`
- 欄位:`資料日期`、`股票代號`、`股票名稱`、**`除權息前收盤價`**、`除權息參考價`、**`權值+息值`**、`權/息`、…
- 注意事項:
  - **日期格式是第四種**:`"113年01月17日"`(民國年中文分隔),與 openapi 的緊湊格式、舊站的斜線格式、ETF 清單的西元年句點格式都不同。已加 `parse_roc_cjk`,不可共用其他解析器。
  - **只取「息」**:`權/息` 欄為「權」時是股票股利,改變的是股數而非現金,當成配息金額算進去會憑空多出一筆錢。
  - **採用此來源的理由不是配息金額**(FinMind 已經有),而是 `除權息前收盤價` —— 那是**當時的真實價格,未經分割還原**。我方的價格序列來自 Yahoo、已被還原,兩者相除就是那個時點的累積分割倍率。沒有它,分割過的標的在股息再投入試算裡會離譜地錯(實測 0050 高估 155.6%)。
  - 實測涵蓋率:10651 筆配息中有 10138 筆取得 `prev_close`(缺的多為上櫃標的,此表只涵蓋上市)。

### SITCA 投信投顧公會 — 基金投資明細(ETF 成分股的來源)
- URL: `https://www.sitca.org.tw/ROC/Industry/IN2629.aspx?pid=IN22601_04`
- 方法: **POST**(ASP.NET WebForms postback),先 GET 取 `__VIEWSTATE` / `__VIEWSTATEGENERATOR` / `__EVENTVALIDATION`
- 回應: HTML 表格。一次請求回傳**一家投信某一類型的全部基金**,各取前十大持股。實測元大 + AH11(國內指數股票型_股票型)得 10 檔基金、100 筆持股
- Fixture: `pipeline/tests/fixtures/sitca_holdings_A0005_AH11_202607.html`
- 欄位:`基金名稱`、`名次`、`標的種類`、`標的代號`、`標的名稱`、`金額`、`占基金淨資產價值之比例(%)`
- **兩個坑**:
  1. **必須帶 `ctl00$ContentPlaceHolder1$rdo1=rbComCL`。** 頁面有三種查詢模式,radio 決定哪一組下拉生效,而沒被選中的那組在瀏覽器裡是 `disabled`、根本不會送出。只送下拉值的話伺服器回**原本的表單頁**,HTTP 200、沒有錯誤訊息,只是查詢沒發生 —— 實測是靠「前後兩次回應的位元組數完全相同」才發現的。
  2. **憑證缺 Subject Key Identifier**,與 `www.tpex.org.tw` 同一個問題(ledger R29),需放寬 `VERIFY_X509_STRICT`。
- 其他注意事項:
  - 同一檔基金的十筆持股,**只有第一列帶基金名稱**,其餘列少一格、整列左移。用固定欄位索引會讓每一檔的第一筆(權重最大的那一筆)被讀成別的欄位。
  - 基金名稱尾端帶著法定公開說明(「(本基金之配息來源可能為收益平準金…)」),必須剝掉才對應得到 ETF 代號;但幣別與級別註記是名稱的一部分,不可一併剝除。
  - 這份資料是**月頻、前十大**,不是每日全持股。對重疊度分析足夠(0050 前五大即佔七成以上權重),但不能宣稱是完整成分股。
  - 同一頁系統下還有「各項費用比率(月、季、年)」`IN2211.aspx?pid=IN2222_01`；本站已另行接上**全年合計費用率**，見下節。

### SITCA 投信投顧公會 — 年度總費用率
- URL:`https://www.sitca.org.tw/ROC/Industry/IN2211.aspx?pid=IN2222_01`
- 實測日期:2026-09-17。查 2025 年「全年」取得 1065 檔基金，其中類型 `AH*`／`AL*` 的 ETF 有 298 檔。
- 表格包含基金統編、基金名稱、交易直接成本、會計帳列費用與「合計」；0050 的 2025 年全年合計為 **0.22%**，存為小數 `0.0022`。
- WebForms 改變年份會重設期間選單，必須先送年份 postback，再選 `Year` 查詢。單月資料如 0050 的 2025 年 12 月 **0.02%** 絕不能誤當全年費率。
- 優先以基金統編對應 TWSE 基金基本資料的證券代號，名稱只作保守備援。實測 298 筆中 196 筆可對上現有代號；其餘保留空值，不用名稱猜測。上櫃債券 ETF 是主要缺口，後續可再尋找官方統編對照來源。
- 只在上一年度資料尚未存入時重查；頁面顯示資料年份。新掛牌基金或全年資料尚未公布時不填 0。

## ETF 成分股:2026-08-26 的勘查結論已被推翻(保留紀錄)

> **2026-08-27 更新:當時判定「沒有任何統一來源」是錯的 —— 漏查了投信投顧
> 公會的「境內基金各項資料」。正確的來源見上方一節。**
> 以下保留原始紀錄,因為那些排除結論本身仍然成立(TWSE、TPEx、FinMind、
> 臺灣指數公司確實都沒有),錯的是「因此不存在」這個推論。



規格 §7.2 的「ETF 成分股重疊度分析」標註為「此階段的唯一新資料需求」。
本次勘查的結論是:**沒有任何統一來源**,只能逐一發行商抓取,而且各家的
取得難度差異很大。以下是實測結果,供日後接續。

### 已排除的統一來源(皆為實測,非推測)

| 來源 | 結果 | 驗證日期 |
|---|---|---|
| TWSE openapi(143 條路徑逐條檢查) | 無任何 ETF 成分股端點。最接近的 `/opendata/t187ap47_L`(基金基本資料彙總表,268 檔上市基金)只有基本資料與標的指數名稱,無持股 | 2026-08-26 |
| TPEx openapi(225 條路徑) | 只有**指數**成分股(`tpex50_constituents`、`tpex_index_consti` 等),沒有 ETF 持股 | 2026-08-26 |
| FinMind(104 個資料集) | `TaiwanStockActiveETFHolding` 需付費層級(免費層回 HTTP 400);且由 `TaiwanStockActiveETFInfo` 實測得知其涵蓋範圍僅 **37 檔主動式 ETF**,不含 0050／0056 | 2026-08-26 |
| SITCA 投信投顧公會 ETF 專區 | 只做各投信網站的連結彙整,不聚合持股 | 2026-08-26 |
| 臺灣指數公司 | 首頁與 `/indexes/TW50` 皆可取得(HTTP 200),但頁面不含成分股代號,需進一步找查詢路徑 | 2026-08-26 |

申購買回清單(PCF)依規定由**各投信自行公告**,這是為什麼沒有統一來源。

### 各發行商的實測難度

依我方資料庫的檔數排序(前三家合計 114 檔,涵蓋絕大多數會被拿來互相比較的標的):

| 發行商 | 檔數 | 端點 | 實測結果 |
|---|---|---|---|
| 富邦 | 41 | `websys.fsit.com.tw/FubonETF/Trade/Pcf.aspx?stkId=006208` | HTTP 200(168 KB)但**持股不在初始 HTML**;ASP.NET,推測需 postback |
| 元大 | 41 | `www.yuantaetfs.com/product/detail/{code}/ratio` | HTTP 200(1.1 MB),持股**部分** server-render:`.tbody .tr` 只有前 5–6 筆,其餘在最小化的 `__NUXT__` payload 內。內部 API 的 FuncId 為 `ETFPCF/PCFRatio`(自 `/_nuxt/*.js` 取得),但 `ectranslation/api/bridge` 缺少必要參數時回 `{"Message":"發生錯誤。"}`,參數組合尚未試出 |
| 國泰 | 32 | `www.cathaysite.com.tw/funds/etf/pcf.aspx?fundid=00878` | HTTP 301 |

元大的 HTML 表格有一個可靠的判別方式:同一頁有兩個 `.tbody` 區塊,
股票區塊的欄位是「商品代碼／商品名稱／商品數量／商品權重」,
期貨區塊是「商品代碼／商品數量／商品權重／**商品年月**」——
以有無「商品名稱」欄即可區分,不必依賴順序。

### 選項 3(改以指數成分股為基礎)也已排除

2026-08-27 追查結果:**臺灣指數公司(TIP)不公開成分股清單**,那是付費商品
(站上有「指數資訊銷售」專區)。

| 探查對象 | 結果 |
|---|---|
| `backend.taiwanindex.com.tw/api/indexes/{code}` | HTTP 200,回傳指數 metadata(編製方式、定審期程、`component_stocks` 只有**檔數**),**無成分股清單** |
| `.../constituents`、`.../components` | HTTP 302 |
| `backend.taiwanindex.com.tw/api/indexes` | HTTP 200,112 KB,全指數清單(仍無成分股) |
| 5 秒資訊檔 `api/downloadFile/SecondInfoFiles/{id}` | HTTP 200,387 KB CSV,欄位為 `生效日,指數代號,指數統計時間hhMMss,指數值` —— 只有**指數值時序**,不含成分股 |
| SITCA「境內基金各項資料」 | ASP.NET 表單頁,無可探查的連結結構 |

這一點很關鍵:我方 235 檔有 `tracking_index`,其中絕大多數是
**臺灣指數公司特選XXX指數**(每檔 ETF 幾乎對應一個專屬指數)。
若 TIP 公開成分股,一個來源就能覆蓋大半;既然不公開,這條路的價值歸零。

### 尚未決定的取捨

1. **逐家爬取**:可行但脆弱。這些都是未公開的私有端點,改版不會有通知。
2. **以 Node 評估 `__NUXT__` payload**:資料完整,但等於在 CI 裡執行遠端
   JavaScript,而且目前的排程 workflow **只裝 Python、沒有 Node**。
3. **改以指數成分股為基礎**:我方已有 235 檔的 `tracking_index`,
   若取得各指數的成分股,重疊度可在指數層級計算,不必碰任何投信網站。
   需要先找到指數成分股的可用來源。
...
4. **(2026-08-26 改判)TWSE MIS 快照可用於當日折溢價。** 它沒有歷史查詢參數，也是預估淨值而非正式結算淨值；但收盤後實測的時間戳與市價/淨值一致性足以支撐當日最終快照。本專案以 SQLite 逐日累積，並以版控內的 `recovery.json` 避免 Actions cache 遺失時永久丟失淨值歷史。
