...
>
> **執行 Task 10 時追加(R24):** `parse_yahoo_chart` 另需捨棄「未調整分割」
> 之前的歷史。實測 0050.TW 在 2014-01-02 有一次 1:4 分割,Yahoo 未回溯調整,
> 且 `events` 中沒有 `splits` 可供偵測 —— 只能靠「單日變動 35% 以上不可能是
> 行情」認出來。未處理時 MDD 算出 −77.3%、波動度灌到 28.4%,兩個都很像真的。
