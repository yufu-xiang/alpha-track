...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  sharpe: {
    ...
    read: '大於 1 算不錯,大於 2 要先懷疑是不是碰上特別好的一年,或樣本期間太短。低於 0 代表報酬還輸給定存,承擔的風險完全沒換到東西。',
    pitfall: '分子只看近一年,所以大多頭年份會讓它異常漂亮 —— 大盤翻倍的一年,連平庸的標的都能有 4 以上。它衡量的是「最近一年賺得值不值」,不是長期效率。空頭期間分子為負,此時數值大小沒有意義(波動越大反而看起來越不糟),只要知道「不好」即可。',
  },
  beta: {
    term: '貝他值(Beta)',
